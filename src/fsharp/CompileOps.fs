...
let OutputPhasedErrorR (os:StringBuilder) (err:PhasedError) =
    let rec OutputExceptionR (os:StringBuilder) = function
...
let OutputPhasedError (os:StringBuilder) (err:PhasedError) (flattenErrors:bool) =
    let buf = new StringBuilder()
        ...
        let writeSigData (ccu:CcuThunk) =
            try
                ...
                | ILResourceLocation.Local bytes ->
                    File.WriteAllBytes(metadataPath + ccu.name + ".sigdata", bytes())
                | _ -> ()
            with Failure msg ->
                printfn "Error: %s" msg
