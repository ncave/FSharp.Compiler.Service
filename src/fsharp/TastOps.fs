...
type CheckCachability<'key,'acc>(name,f: FreeVarOptions -> 'key -> 'acc -> bool * 'acc) =
    let dict = System.Collections.Generic.Dictionary<'key,int>(3, HashIdentity.Reference)
    let idem = System.Collections.Generic.Dictionary<'key,int>(3, HashIdentity.Reference)
    let closed = System.Collections.Generic.Dictionary<'key,int>(3, HashIdentity.Reference)
    let mutable saved = 0
...
let mkAnyTupledTy g tupInfo tys = 
    match tys with 
    | [] -> g.unit_ty 
    | [h] -> h
